...
router.put('/images/*', requirePassword, async (req, res) => {
    ...
    try {
        ...
        // 安全校验：newName 不允许路径穿越
        let targetFilename = oldFilename;
        if (newName) {
            const trimmed = newName.trim();
            if (!trimmed) return res.status(400).json({ success: false, error: '新文件名不能为空' });
            const safeBase = path.basename(trimmed);
            if (!safeBase || safeBase !== trimmed) return res.status(400).json({ success: false, error: '非法文件名' });
            targetFilename = sanitizeFilename(safeBase);
        }
